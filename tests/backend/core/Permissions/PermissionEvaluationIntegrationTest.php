...
class PermissionEvaluationIntegrationTest extends FunctionalTestCase
{
    ...
    public function testUserWithoutSpecificPermissionCannotAccessRestrictedFeature(): void
    {
        ...
        $this->logIn($this->testUser->_real());
        $this->ensurePermissionsInitialized($this->testUser->_real());
    ...
    public function testUserWithSpecificPermissionCanAccessRestrictedFeature(): void
    {
        ...
        $userPermissions = $this->userAccessControlService->getUserPermissions($this->testUser->_real());
    ...
    public function testUserSpecificPermissionOverridesRoleBasedDenial(): void
    {
        ...
        $foundPermissions = $this->userAccessControlService->getUserPermissions($this->testUser->_real());
        self::assertCount(1, $foundPermissions, 'Should find exactly one user permission');

        // Debug: Check user relationships
        self::assertNotNull($this->testUser->_real()->getOrga(), 'User should have organization');
        self::assertNotNull($this->testUser->_real()->getCurrentCustomer(), 'User should have current customer');
    ...
    public function testUserSpecificPermissionsDoNotAffectOtherUsers(): void
    {
        ...
        $otherUser->_real()->setOrga($otherOrga->_real());
        $otherOrga->_real()->addUser($otherUser->_real());
        $otherUser->_real()->addDplanRole($this->testRole);
        ...
        // Use the addCustomerAndOrgaType method to establish the relationship properly
        $otherOrga->_real()->addCustomerAndOrgaType($otherCustomer->_real(), $otherOrgaType);

        // Set the current customer for the other user
        $otherUser->_real()->setCurrentCustomer($otherCustomer->_real());

        // Persist all changes
        $this->getEntityManager()->persist($otherUser->_real());
        $this->getEntityManager()->persist($otherOrga->_real());
        $this->getEntityManager()->flush();
        ...
        $this->logIn($otherUser->_real());
        $this->ensurePermissionsInitialized($otherUser->_real());
        ...
        $otherUserPermissions = $this->userAccessControlService->getUserPermissions($otherUser->_real());

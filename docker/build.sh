...
DOCKER_BUILDKIT=1 docker build --build-arg PROJECT_NAME=$projectname --build-arg BUILD_MODE=$build_mode --secret id=envlocal,src=../.env.local --secret id=ado,env=ADO_NPM_AUTH_IDENT -t $imagename:$version -f $folder/Dockerfile --target fpm $context

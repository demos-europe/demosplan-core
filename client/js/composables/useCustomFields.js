...
/*
 * Resource-type → URL builder.
 * DraftStatement is migrated to API Platform 3.0; other types stay on the EDT
 * (api/2.0) routes generated by Symfony Routing.
 */
function buildResourceUrl (action, resourceType, resourceId) {
if (resourceType === 'DraftStatement') {
  const idSegment = resourceId ? `/${resourceId}` : ''

  return `/api/3.0/DraftStatement${idSegment}`
}
  ...
  const routeName = {
    ...
    list: 'api_resource_list',
    update: 'api_resource_update',
  }[action]

  return Routing.generate(routeName, resourceId ? { resourceType, resourceId } : { resourceType })
}
...
function fetchIndividualValues (resourceType, resourceId) {
  ...
  const url = buildResourceUrl('get', resourceType, resourceId)
  const params = {
    fields: {
...
function fetchBatchValues (resourceType, definitionSourceId, cacheKey, filterPath) {
  const url = buildResourceUrl('list', resourceType)
  const params = {
    fields: {
      [resourceType]: ['customFields'].join(),
    },
...
export function useCustomFields () {
  ...
  const updateCustomFields = (resourceType, resourceId, customFieldValues) => {
    const url = buildResourceUrl('update', resourceType, resourceId)

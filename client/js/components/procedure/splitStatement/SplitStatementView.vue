      ...
      <dp-sticky-element border>
        <header
          ...
          <dp-inline-notification
            ...
            type="info" />
          <h1 class="font-size-h1 u-valign--bottom display--inline-block u-m-0">
        ...
        <div v-if="segmentationStatus === 'inUserSegmentation'">
          ...
          <main
            ...
            <transition
              ...
              mode="out-in">
              <card-pane
                v-if="showTags && editModeActive === false"
                id="cardPane"
                class="u-ml"
                :key="tagsCounter"
            ...
            <transition
              ...
              <dp-sticky-element
                ...
                :apply-z-index="false"
                :context="$refs.main"
                :offset="58/* The height of the sticky #header + 6px to have a little margin between both elements when fixed */">
                <side-bar
                  id="sideBar"
                  class="u-mb-0_25"
                  ref="sideBar"
...
import { DpFlyout, DpInlineNotification, DpStickyElement } from '@demos-europe/demosplan-ui/components/core'
...
import SideBar from '@DpJs/components/procedure/splitStatement/SideBar'
      ...
      return dpApi.get(url, { sort: 'lastname' }, { serialize: true })
        .then(response => {
          this.assignableUsers = response.data.data.map(assignableUser => {
            return {
              label: assignableUser.attributes.firstname + ' ' + assignableUser.attributes.lastname,
              value: assignableUser.id
            }
          })
          this.assignableUsers.unshift({ label: Translator.trans('not.assigned'), value: 'noAssigneeId' })
  ...
  mounted () {
    this.fetchStatementSegmentDraftList(this.statementId)
      .then(({ data }) => {
         ...
         */
        if (data.data.attributes.segmentDraftList?.data.attributes.segments.length > 0) {
          this.fetchInitialData().then(() => {
            this.segmentationStatus = 'aiSegmented'
          })
        } else {
